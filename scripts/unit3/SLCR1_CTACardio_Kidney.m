%[text] # Slicer Volume Visualization - CTACardio Kidney
%[text] In this module, we will load the CTACardio volume and Kidney segmentation that we created in Slicer.  Remember, segmentation files are label maps that have the same dimensions as the original volume, but have labels (numbers) instead of intensity values. There is a label value for each structure segmented from the original file. So, if there four structures segmented, there will be 4 labels in the volume (labels 1-4).
...
paths.project = "/Users/ernesto/MATLAB-Drive/MtMdata/unit3/CTACardio"; %[control:filebrowser:3f76]{"position":[17,70]}
ls(paths.project) % display folder contents
paths.intensity = fullfile(paths.project,"CTACardioCrop.nrrd");
paths.segment = fullfile(paths.project,"Segmentation.seg.nrrd")
%[text] path check
...
volshow(mVint.Voxels.pixelData,Parent=hvr3,RenderingStyle="GradientOpacity",OverlayData=mVseg.Voxels.pixelData,Transformation=tform)
%%
%[text] ### Use Slicer colors
segT = mmGetSlicerSegTable(paths.segment)
%%
hvr3 = viewer3d(BackgroundColor="white",BackgroundGradient="off",CameraZoom=1); % create new viewer window
volshow(mVint.Voxels.pixelData,Parent=hvr3,RenderingStyle="GradientOpacity",...
    OverlayData=mVseg.Voxels.pixelData,...
    OverlayColormap=segT.Color,...
    OverlayAlpha=0.5,...
    Transformation=tform)
%%
figure
MASK = mVseg.Voxels.pixelData == 1;
hp = mmPlotMask2Surface(MASK,affTrfm=tform.A)
